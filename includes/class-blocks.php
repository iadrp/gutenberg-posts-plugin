...
class Blocks {
	...
	public function register_blocks() {
		...
		foreach ( $this->blocks as $block => $dir ) {
			...
			$args = [
				...
				'editor_script' => 'GPB-editor-js',
				// Editor stylesheet.
				'editor_style'  => 'GPB-editor-css',
				// Blocks stylesheet.
				'style'         => 'GPB-blocks-css',
			];
			...
			// Register block.
			register_block_type( 'GPB-blocks/' . $block, $args );
	...
	public function editor_assets() {
		...
		wp_register_style(
			...
			GPB_URL . 'assets/editor.build.css',
			// Gutenberg dependency, to include the CSS after it.
			[ 'wp-edit-blocks' ],
			// To prevent cache issue (can be the plugin version in production).
			filemtime( GPB_PATH . 'assets/editor.build.css' )
		);
		...
		wp_register_script(
			...
			GPB_URL . 'assets/editor.build.js',
			// Gutenberg dependencies, to include the JS after it.
			[ 'wp-blocks', 'wp-element', 'wp-i18n' ],
			// To prevent cache issue (can be the plugin version in production).
			filemtime( GPB_PATH . 'assets/editor.build.js' )
		);
	...
	public function i18n_register() {
		...
		// Get translations.
		$locale  = gutenberg_get_jed_locale_data( 'gutenberg-posts-block' );
		// Add translations to wp.i18n.setLocaleData.
		$content = 'wp.i18n.setLocaleData(' . json_encode( $locale ) . ', "gutenberg-posts-block" );';

		// Add inline script before 'GPB-editor-js' script.
		wp_script_add_data( 'GPB-editor-js', 'data', $content );
